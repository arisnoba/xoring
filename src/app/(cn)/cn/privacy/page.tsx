			...
			<div className="mx-auto max-w-4xl px-4 py-32 pt-40 pb-32 font-sans">
				...
				<div className="prose prose-invert max-w-none text-white/80 leading-7 space-y-8">
					<section className="space-y-4">
						...
						<p>本政策说明我们如何收集、使用、共享、保存和保护用户及其他个人与平台相关的个人信息。如果你不同意本政策，请勿使用平台。</p>
					</section>
					...
					<section className="space-y-6">
						...
						<div className="space-y-6 pl-4">
							...
							<div>
								...
								<p>我们可能根据 SIM 卡及／或 IP 地址收集大致位置信息。在取得你的同意后，我们也可能收集 GPS 等精确位置信息，用于运动记录、移动距离计算、路线展示及挑战验证等功能。</p>
					...
					<section className="space-y-4">
						...
						<p>我们可能根据上述行为数据计算并发放 AIOS 代币或其他数字奖励。具体累积方式、计算标准、发放周期、使用范围及失效条件，以另行制定的 AIOS 代币运营政策或相关运营政策为准。</p>
						<p>AIOS 代币或其他奖励不等同于现金或法定货币，不应被理解为保证的投资收益或现金兑换工具。对于通过不当行为获得的奖励，我们可予以追回或限制相关账户。</p>
						<h3 className="text-lg font-medium text-white">数据处理方式</h3>
						<p>我们收集的行为数据可能经过假名化或匿名化处理，使个人无法被直接识别；部分数据可能汇总后用于统计和研究。你可以通过平台设置或相关程序管理对行为数据收集和使用的同意，撤回同意可能导致部分功能或奖励累积受到限制。</p>
					</section>
					...
					<section className="space-y-4">
						...
						<p>我们会在向你提供平台服务及实现本政策所述目的所必需的期间内保存个人信息。为履行合同或法律义务、解决争议、应对滥用、保障安全，或提出、行使和抗辩法律请求，我们也可能继续保存相关信息。</p>
						<p>保存期限会因信息类型、收集目的及法律要求而不同。例如，账户信息可在账户存续期间保存；发生违反条款或政策的情况时，部分信息可能为调查和应对目的保存更长时间。</p>
					</section>

					<section className="space-y-4">
						<h2 className={headingClassName}>儿童与青少年相关信息</h2>
						<p>原则上，XO RING 不面向未满 14 周岁的儿童。相关司法管辖区的法律可能规定更高的最低年龄。</p>
						<p>如果我们确认低于适用最低年龄的用户正在使用平台，可能会限制或删除账户，并相应处理相关信息。</p>
					</section>

					<section className="space-y-4">
						<h2 className={headingClassName}>本隐私政策的更新</h2>
						<p>我们可能根据需要不时更新本政策。更新时，我们会修改上方的“最后更新”日期，并通过发布更新后的政策或适用法律要求的其他方式通知你。</p>
					...
					<section className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-6 mt-16">
						...
						<p>我们将尽快处理你的请求。这不限制你依据适用法律向相关监管机构投诉的权利。</p>
					</section>
					...
					<section className="space-y-6 mt-20">
						...
						<div className="space-y-6 pl-4">
							<div>
								...
								<p>收集和使用目的达成或保存期限届满后，我们将立即销毁个人信息。但适用法律要求保存时，我们会在法定期限内保存，包括：</p>
								<ul className={listClassName}>
							...
							<div>
								...
								<p>我们可能委托国内外服务提供商处理个人信息，或为云存储、分析、客户支持、安全、身份验证及奖励运营等目的跨境传输个人信息。发生此类处理时，我们将依据适用法律采取必要的合同、技术和管理保护措施。</p>
							</div>
							<div>
