				...
				<div className="prose prose-invert max-w-none text-white/80 leading-7 space-y-8">
					...
					<section className="space-y-4">
						...
						<ul className={listClassName}>
							...
							<li>用户使用服务，即视为同意本条款、隐私政策、社区运营政策，以及公司另行制定的包括 AIOS 代币或奖励相关政策在内的其他政策。上述各项政策均构成本条款不可分割的一部分。</li>
							<li>服务可能在韩国及其他国家或地区提供。根据各适用司法管辖区的法律法规，可能适用附加条款；如本条款与特定司法管辖区的附加条款发生冲突，以该附加条款为准。</li>
						</ul>
					</section>
					...
					<section className="space-y-4">
						...
						<ul className={listClassName}>
							...
							<li>公司修改本条款时，将通过服务内公告或其他合理方式，提前说明修改内容、生效日期及修改理由。若修改内容对用户具有重大不利影响或属于其他重要事项，公司将在合理期限内另行提前通知。</li>
					...
					<section className="space-y-4">
						...
						<p>用户可能需要创建账户才能使用全部或部分服务。用户应提供准确、最新的信息，并在信息发生变化时及时更新。</p>
						<p>用户有责任妥善保管账户凭据和密码，不得与任何第三方共享。除因公司故意或重大过失造成的情况外，用户应对通过其账户进行的活动负责。</p>
						...
						<p>用户申请注销账户时，公司将依据适用法律及内部政策处理。账户注销后，部分或全部活动记录、奖励记录、徽章及设置可能无法恢复。</p>
					</section>
					...
					<section className="space-y-4">
						...
						<p>服务可与智能戒指等外部设备配合使用。用户理解并同意，部分功能或测量值可能因设备连接状态、电池状态、传感器精度、操作系统环境及网络状况而有所不同。</p>
						<p>服务展示的活动及健康相关数据仅作为一般信息和个人记录管理的参考。服务不能替代医疗行为、诊断、治疗、紧急处置或专业医疗建议。</p>
						<ul className={listClassName}>
					...
					<section className="space-y-4">
						...
						<p>公司可能将上述数据用于提供运动记录、改善健康管理便利性、提供个性化统计与反馈、运营挑战和发放徽章、检测异常使用和防止奖励欺诈、提升服务质量和系统稳定性、计算 AIOS 代币或其他奖励，以及履行法律义务。具体收集项目、处理方式、保存期限及向第三方提供等事项，以隐私政策为准。</p>
					</section>
					...
					<section className="space-y-4">
						...
						<p>公司可为用户提供参与挑战和获取徽章的机会。挑战类型、参与要求、达成标准、期限及是否提供奖励，可依据公司政策而有所不同。</p>
					...
					<section className="space-y-4">
						...
						<p>公司将尽合理努力稳定提供服务。但受设备连接状态、传感器特性、网络状况、操作系统环境及其他技术条件影响，服务可能受到限制或测量值可能存在误差。公司不保证服务始终不中断、所有测量数据完全准确、任何特定运动成果、健康改善或奖励结果必然实现，也不保证所有错误或缺陷均能立即修复。</p>
					...
					<section className="space-y-4">
						...
						<p>在适用法律允许的范围内，公司不对因使用服务而产生的间接损失、特殊损失、后果性损失、经营损失或预期利润损失承担责任。但因公司故意或重大过失造成的损失除外，并依适用法律处理。</p>
					...
					<section className="space-y-4">
						...
						<p>原则上，本服务面向年满 14 周岁的用户。若适用司法管辖区的法律规定更高的最低年龄要求，以该要求为准。</p>
