...
export default function Header({ locale, copy }: { locale: Locale; copy: SiteMessages['header'] }) {
	...
	return (
		<header className={cn('fixed top-0 left-0 right-0 z-50 h-(--header-height-mobile) pl-(--safe-area-left) pt-(--safe-area-top) pr-(--safe-area-right) lg:h-(--header-height-desktop)', themeTransitionClassName, headerSurfaceClassName)}>
