...
function SocialCard({ title, description, theme }: { title: string; description: string; theme: string }) {
	return (
...
export default function SocialModeSection() {
	return (
		<section id="ring" className="relative isolate overflow-hidden bg-[#f2f2ef] text-[#111]">
						...
						<div className="mt-20 overflow-x-auto pb-6">
							<div className="flex w-max gap-4 pr-8 xl:gap-5">
								{SOCIAL_MODE.cards.map((card, index) => (
									<SocialCard key={card.title} title={card.title} description={card.description} theme={socialArtThemes[index % socialArtThemes.length]} />
								))}
							</div>
						</div>
					</div>
